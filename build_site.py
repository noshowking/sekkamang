...
# 확률 예측 가중치 [wdRecent(최근 같은요일), wdRate(전체 요일), recentRate(최근30일 빈도), durTr(직전날 전이/연속성)] — 백테스트로 튜닝
# wr(전체요일)은 예측력 없어 0으로 제거, 그 비중을 durTr(연속성)로 이전. holdout 적중률 72%→74%
PRED_WEIGHTS = [0.10, 0.0, 0.50, 0.40]
